    ...
    var body: some View {
        NavigationSplitView {
            ...
            GeometryReader { _ in
                ...
                case .smartWifi:
                    SmartWifiView(smartWifiViewModel: SmartWifiViewModel<AppSmartWifiService>(appSmartWifiService: AppSmartWifiService(serviceKey: "AppleBCMWLANSkywalkInterface"), systemPreferenceService: SystemPreferenceService(), appCoreWLanWifiService: AppCoreWLanWifiService(wifiClient: CWWiFiClient.shared(), wifyKeyChainService: AppWifiKeyChainService()), appSettingService: AppStorageSetting(), appWifiMonitoringService: AppSmartWifiMonitoringService(wifiClient: CWWiFiClient())))
                        .environmentObject(colorSchemeViewModel)
